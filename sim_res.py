...
result = sim_params(tmp, outName='pres_sim_typ', temp=27, gateVoltage=1.5, process='tt');
print('Typical case:')
...
for temp in temps:
  for vdd in vdds:
    for proc in procs:
      ...
      result = sim_params(tmp, outName=name, temp=temp, gateVoltage=vdd, vdd=vdd, process=proc);
